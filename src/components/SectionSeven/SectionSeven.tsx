...
export const SectionSeven = () => {
	...
	return (
		<Box bgColor={'yellow'}>
			<Box bgColor={'black'} h={'324px'} w={'1000px'} p={'70px 45px 0px 45px'} m="0 auto">
				<Flex flexDir={'column'} justifyContent={'center'} alignItems={'start'}>
					<Text color="white" mb={'30px'}>
						Questions? call 0-800-666-2808
					</Text>
					<Flex maxW={'900px'} h={{ md: '128px' }}>
						{arr.map((item, idx) => {
							return (
								<Text key={idx} color={'white'} w={'25%'}>
									{item}
								</Text>
							);
						})}
					...
					<Text color="white">Netflix Argentina</Text>
				</Flex>
			</Box>
		</Box>
	);
